...
n = 999999 # Increased size for better performance in parallel
A = np.random.rand(n)
B = np.random.rand(n)
...
start_time = time.time()
result_parallel = parallel_dot_product(A, B, num_processors)  # Use 4 processors instead of 8
...
start_time = time.time()
result_parallel_threads = parallel_dot_product_threads(A, B, num_processors)  # Use 4 threads
...
# Plotting execution times, speed-up, and efficiency